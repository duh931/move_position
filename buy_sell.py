...
def get_contract_type():
	...
	for names in allFiles:
		match=re.search(pattern,names).group(0)
		name_list.append(match)
	return set(name_list)
	
	
	
def value_calculator(contract_type,time_period='*',time_scale='1 Hour',days_before_expiration=30,days_to_start=90):	
	dd = dbWrapper('192.168.10.188')

	principal=500000.0
	allFiles = glob.glob(time_period+"/*"+contract_type+"*.csv")
	...
	if last_row['Date1'].values==last_row['Date2'].values and last_row['Time1'].values==last_row['Time2'].values:
		...
		if int(expire_date[:2])!=1:
			date_end=datetime(year=date_end.year,month=int(expire_date[:2]),day=1)
		else:
			date_end=datetime(year=date_end.timetuple().tm_year+1,month=int(expire_date[:2]),day=1)
	...
	buy_sell=np.zeros([len(all_commodity),duration])
	...
	for index, row in all_trades.iterrows():
		if int(row['settle']/100.0)!=1:
			expire_date=datetime(year=row['Date2'].year,month=int(row['settle']/100.0),day=1)			
		else:
			expire_date=datetime(year=row['Date2'].timetuple().tm_year+1,month=int(row['settle']/100.0),day=1)
		if int(toString(expire_date))>int(toString(end_date)):
			expire_date=end_date
		if row['Date1']<(expire_date-timedelta(days=days_before_expiration)) and row['Date1']>=(expire_date-timedelta(days=days_to_start)):
			...
			if time1==time2:
				time2=expire_date-timedelta(days=days_before_expiration)
			...
			buy_sell[commodity_index,time1_index]+=row['Buy']
			buy_sell[commodity_index,time2_index]-=row['Buy']
			

	buy_sell_sum=buy_sell[0]
	for i in range(1,len(all_commodity)):
		if i%2==0:
			buy_sell_sum+=buy_sell[i]
	...
	df.to_csv('../on_off_data/'+contract_type+str(days_to_start)+'_'+str(days_before_expiration)+'.csv')
	print ('saved to '+contract_type+str(days_to_start)+'_'+str(days_before_expiration)+'.csv')
	...
	fig,ax=plt.subplots()
	ax.plot(date_list,net_worth_sum)
	scale=np.max(net_worth_sum)-np.min(net_worth_sum)
	for i in range(duration):
		if buy_sell_sum[i]>0:
			x=date_list[i]
			y=net_worth_sum[i]
			dx=0.0
			dy=scale/30.0*buy_sell_sum[i]
			ax.arrow(x,y-dy,0,dy,head_width=0.75,head_length=dy/2.0,length_includes_head=True)
		if buy_sell_sum[i]<0:
			x=date_list[i]
			y=net_worth_sum[i]
			dx=0.0
	...
	fig.savefig('../on_off_plot/'+contract_type+str(days_to_start)+'_'+str(days_before_expiration)+'.png',dpi=200)
	plt.close(fig)
	return 0

if __name__ == '__main__':
	contract_list=get_contract_type()
	for contract in contract_list:
		for i in range(90,50,-10):
			for j in range(10,40,10):
				value_calculator(contract,days_before_expiration=j,days_to_start=i)
	# value_calculator('spc_',time_scale='1 Hour',time_period='*',days_before_expiration=60)
